...
export const XYAxis: React.FunctionComponent<XYAxisProps> = ({x, y, children}) => {
    ...
    const yTicks = newYScale.ticks(yAxis.tickTextSize === 0 ? yTicksCount : Math.floor(newHeight / yAxis.tickTextSize)).map((tick: number) => (
        <g
    ...
    const xNumberOfTicks = xLinearScale.ticks(xAxis.tickTextSize === 0 ? xTicksCount : Math.floor(newWidth / xAxis.tickTextSize)).length;
    const ticks = _.chunk(newXScale.domain(), Math.floor(newXScale.domain().length / xNumberOfTicks))
        ...
        .filter((arr: number[], index: number, arrays: number[][]) => index < arrays.length - 1 || arr.length > 1 || arrays[0].length === 1)
    ...
    const xTicks = newXScale.domain().map((tick: number) => {
        ...
        return (
            <g key={`x-axis-tick-${tick}`} transform={`translate(${textX + xInnerPadding},${minY + 2 * yInnerPadding})`}
    ...
    return (
        <>
            ...
            {yAxis.show && (
                <g className='y-axis'>
                    <line className='axis-line' stroke='black' x1={maxX + 2 * xInnerPadding} x2={maxX + 2 * xInnerPadding} y1={minY + 2 * yInnerPadding} y2={maxY} />
                    {yTicks}
                </g>
            )}
            {xAxis.show && (
                <g className='x-axis'>
                    <line className='axis-line' stroke='black' x1={minX} x2={maxX + 2 * xInnerPadding} y1={minY + 2 * yInnerPadding} y2={minY + 2 * yInnerPadding} />
                    {xTicks}
                </g>
            )}
        </>
