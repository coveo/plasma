...
export const InfoLine: React.FunctionComponent<InfoLineProps> = ({stroke = '#000', isVertical = false, label = '', padding = 30, value}: InfoLineProps) => {
    ...
    return (
        <g className='info-line'>
            ...
            />
            {label && !isVertical && <text textAnchor='end' x={xScale(xDomain[1])} y={yScale(value) - 4}>{label}</text>}
            {label && isVertical && <text textAnchor='end' transform='rotate(-90)' x={yScale(yDomain[1])} y={xScale(value) - 4}>{label}</text>}
        </g>
    );
};
