...
- `enableMultiRowSelection` MAY be a predicate when some rows should only support single selection. Rows rejected by the predicate are displayed at 50% opacity and do not render a selection checkbox. They are skipped by select-all and range selection, but remain selectable through their surface and continue to trigger `onRowDoubleClick`. Selecting one clears the current selection, and selecting another row afterward clears the single-select-only row.
