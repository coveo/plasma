...
describe('TextAreaLabel', () => {
    it('should render without error with TextArea or TextAreaConnected inside it', () => {
        expect(() => shallow(<TextAreaLabel label='label'><TextArea id='test' /></TextAreaLabel>)).not.toThrow();
        expect(() => shallow(<TextAreaLabel label='label'><TextAreaConnected id='test' /></TextAreaLabel>)).not.toThrow();
    });

    it('should render with a container div containing the textarea and a label', () => {
        const containerDiv = shallow(<TextAreaLabel label='label'><TextArea id='test' /></TextAreaLabel>).find('div');
    ...
    it('should have a label with the same id as the textarea', () => {
        ...
        const containerDiv = shallow(<TextAreaLabel label='label'><TextArea id={textAreaId} /></TextAreaLabel>).find('div');
    ...
    it('should render with extra classes on the container div if passed as prop', () => {
        ...
        let component = shallow(<TextAreaLabel label='label'><TextArea id='test' /></TextAreaLabel>);
        expect(component.find('div').prop('className')).not.toContain(extraClass);

        component = shallow(<TextAreaLabel label='label' containerClassName={extraClass}><TextArea id='test' /></TextAreaLabel>);
