...
const mapStateToProps = (state: IReactVaporState, ownProps: IFlatSelectOwnProps): IFlatSelectStateProps => {
  ...
  if (flatSelectState && flatSelectState.selectedOption) {
    return { selectedOption: flatSelectState.selectedOption };
  }

  return { selectedOption: ownProps.options.length && ownProps.options[0] };
};

const mapDispatchToProps = (dispatch: (action: IReduxAction<IReduxActionsPayload>) => void,
  ownProps: IFlatSelectOwnProps): IFlatSelectDispatchProps => ({
    onMount: () => dispatch(addFlatSelect(ownProps.id, ownProps.options.length && ownProps.options[0])),
    onDestroy: () => dispatch(removeFlatSelect(ownProps.id)),
    onOptionClick: (selected: IFlatSelectOptionProps) => dispatch(selectFlatSelect(ownProps.id, selected)),
  });
